...
var baseCss = "https://lihautan.com/notes/async-initialisation-nodejs-lib/assets/blog-base-248115e4.css";
...
function create_fragment$1(ctx) {
	...
	return {
		...
		h() {
			...
			attr(meta8, "content", "https%3A%2F%2Flihautan.com%2Fnotes%2Fasync-initialisation-nodejs-lib");
			attr(a, "href", "#content");
...
function create_default_slot(ctx) {
	...
	let raw0_value = `<code class="language-json"><span class="token comment">// package.json</span>
...
<span class="token punctuation">&#125;</span></code>` + "";
	...
	let raw1_value = `<code class="language-js"><span class="token comment">// >= node 14 (with top-level await support)</span>
...
<span class="token punctuation">&#125;</span><span class="token punctuation">)</span><span class="token punctuation">(</span><span class="token punctuation">)</span><span class="token punctuation">;</span></code>` + "";
	...
	return {
		c() {
			...
			p = element("p");
			t0 = text("I have a library that needs to be called synchronously but has an async initialization step.");
			br0 = element("br");
			br1 = element("br");
			t1 = text("Can I use pkg.exports or something else to use top-level await when supported, and fallback to export the initialization promise so that Node.js 12-13 users can await it themselves?");
			t2 = text("— Nicolò Ribaudo 🏳️‍🌈 (@NicoloRibaudo) ");
			a = element("a");
			t3 = text("September 12, 2020");
			t4 = space();
			pre0 = element("pre");
			t5 = space();
			pre1 = element("pre");
		...
		l(nodes) {
			...
			p = claim_element(blockquote_nodes, "P", { lang: true, dir: true });
			var p_nodes = children(p);
			t0 = claim_text(p_nodes, "I have a library that needs to be called synchronously but has an async initialization step.");
			br0 = claim_element(p_nodes, "BR", {});
			br1 = claim_element(p_nodes, "BR", {});
			t1 = claim_text(p_nodes, "Can I use pkg.exports or something else to use top-level await when supported, and fallback to export the initialization promise so that Node.js 12-13 users can await it themselves?");
			p_nodes.forEach(detach);
			t2 = claim_text(blockquote_nodes, "— Nicolò Ribaudo 🏳️‍🌈 (@NicoloRibaudo) ");
			a = claim_element(blockquote_nodes, "A", { href: true });
			var a_nodes = children(a);
			t3 = claim_text(a_nodes, "September 12, 2020");
			a_nodes.forEach(detach);
		...
		h() {
			attr(p, "lang", "en");
			attr(p, "dir", "ltr");
			attr(a, "href", "https://twitter.com/NicoloRibaudo/status/1304710001982877697?ref_src=twsrc%5Etfw");
...
const metadata = {
	...
	"tags": ["nodejs"],
	"slug": "notes/async-initialisation-nodejs-lib",
	"type": "notes",
	"name": "async-initialisation-nodejs-lib",
	"layout": "note"
};
