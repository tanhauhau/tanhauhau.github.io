<script lang="ts">
  ...
  function slide(node: Element, params: any) {
    return {
      ...params,
  ...
  function lockBody(show: boolean) {
    if (show) {
  ...
  {#if show}
    <div class="panel" transition:slide>
      <ul class="links">
        ...
        <li><a onclick={hide} href="/blogs">Writings</a></li>
        <li><a onclick={hide} href="/talks">Talks</a></li>
        <li><a onclick={hide} href="/notes">Notes</a></li>
        <li><a onclick={hide} href="/books">Books</a></li>
        <li class="social">
      ...
      {#if tableOfContents?.length > 0}
        ...
        <ol>
          {#each tableOfContents as { link, title, nested: n1 }}
            <li>
              <a onclick={hide} href="#{link}">{title}</a>
              {#if n1}
                <ol>
                                      ...
                                      {#if n4}
                                        <ol>
                                          {#each n4 as { link, title }}
                                            <li><a onclick={hide} href="#{link}">{title}</a></li>
                                          {/each}
                                        </ol>
                                      {/if}
                                    </li>
    ...
    <div class="backdrop" onclick={() => (show = false)} transition:fade></div>
  {:else}
    <!--svelte-ignore a11y_no_static_element_interactions -->
    <!--svelte-ignore a11y_click_events_have_key_events -->
    <div class="float" onclick={() => (show = true)} transition:slide>
