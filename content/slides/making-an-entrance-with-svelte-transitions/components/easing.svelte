<script>
	import { bounceOut, bounceIn, bounceInOut, quadIn, quadOut, quadInOut, cubicIn, cubicInOut, cubicOut, quartIn, quartInOut, quartOut } from 'svelte/easing';
...
<div class="left">
  ...
  <div class="square" class:hidden={i < 8} style="transform: translateX({ (i === 9 || i === 8 ? u : i === 10 ? (1 - u) : 0) * 250 }px)">
    ...
    <br />
		...
		<div class="code" class:hidden={i < 5} use:prismJs={`let t = Date.now() - start; // ${throttle(duration)}`} />
		<div class="code" class:hidden={i < 6} use:prismJs={`t = t / duration; // ${t.toFixed(3)}`} />
    <div class="code" class:hidden={i < 7} use:prismJs={`t = ${fn.fn.name}(t); // ${u.toFixed(3)}`} />
    <br />
    <div class="code" class:none={i !== 8} use:prismJs={`node.style.transform = \`translateX(\${t * 250}px)\`; // transformX(${(u * 250).toFixed(1)}px)`} />
    <div class="code" class:none={i !== 9} use:prismJs={`css: (t, u) => \`translateX(\${t * 250}px)\``} />
    <div class="code" class:none={i !== 10} use:prismJs={`css: (t, u) => \`translateX(\${u * 250}px)\``} />
    <div class="code" class:none={i !== 11} use:prismJs={`tick: (t, u) => node.textContent = t`} />
    <div class="code" class:none={i !== 12} use:prismJs={`const string = 'Hello World';\ntick: (t, u) => {\n  node.textContent = string.slice(0, Math.round(string.length * t));\n}`} />
	</div>
</div>
